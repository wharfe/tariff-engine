...
**重要: 候補の横断評価**
ヘディングフィルタを通過した全ヘディングについて、それぞれ配下のサブヘディングを評価し、最終的に全候補のfinalConfidenceでソートして上位3件を返す。ヘディング1位の配下だけを評価するのではなく、フィルタ通過した全ヘディング×全サブヘディングの組み合わせを評価対象とする。これにより、headingScoreでは2位だがsubheadingScoreで逆転するケースを正しく拾える。
