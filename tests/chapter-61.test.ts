...
describe("Chapter 61 accuracy (without rules)", () => {
  beforeAll(() => {
    loadTree();
    clearRules();
  });
  ...
  it.each(testCases as TestCase[])(
    ...
    ({ description, expected4 }) => {
      ...
      if (!match4) {
        console.log(
          `  MISS: "${description}" -> got ${top?.hscode ?? "none"}, expected ${expected4}*`,
        );
      }
  ...
  it("reports accuracy", () => {
    console.log(`\n  4-digit: ${correct4}/${total} (${((correct4 / total) * 100).toFixed(0)}%)`);
    console.log(`  6-digit: ${correct6}/${total} (${((correct6 / total) * 100).toFixed(0)}%)\n`);
  });
});
